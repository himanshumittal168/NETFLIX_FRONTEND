...
const Anime = () => {
  ...
  const getTvShows = async () => {
    try {
      setIsLoading(true);
      const response = await fetch("https://api.themoviedb.org/3/discover/tv?api_key=9c869b656bbcb8317a0c240b21095641&with_genres=16");
      if (!response.ok) {
  ...
  const addFavorites = async (tvShow) => {
    try {
      ...
      toast.success(`${tvShow.name} added to favorites`);
    } catch (error) {
      toast.error('Failed to add TV show to favorites. Please try again.');
    }
  };
  ...
  return (
    <div className="movie-list-container my-8">
      ...
      <div className="flex flex-row justify-between">
        <h2 className="text-2xl font-bold text-white mb-4 px-4 uppercase">Anime</h2>
